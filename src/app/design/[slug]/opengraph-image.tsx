...
export default async function Image({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return new ImageResponse(
    (
      <div style={{ width: "100%", height: "100%", display: "flex", background: "#f1f1f1", color: "#161616", fontFamily: "Geist" }}>
        <div style={{ display: "flex", flexDirection: "column", justifyContent: "space-between", width: landscape ? 420 : 560, padding: "56px 48px 56px 64px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            <div style={{ display: "flex", fontSize: 20, color: "#6b6b6b" }}>UI design</div>
            <div style={{ display: "flex", fontSize: 54, fontWeight: 500, letterSpacing: -2, lineHeight: 1.05 }}>{design.title}</div>
            <div style={{ display: "flex", fontSize: 20, color: "#6b6b6b" }}>{formatDate(design.date)}</div>
          </div>
        </div>
